...
# -*- coding: utf-8 -*-
"""
SAM 交互 + TXT 区域（YOLOv11 输出）自动框选 + 最平滑曲线(K) + 原 DP 折线(E)
不再使用手动 B/X 框选，直接读取与 IMAGE_PATH 同名的 .txt 文件中给出的区域。
其他 S/K/E/J/C/R/Q 功能保持不变，按 D 切换多个检测框（如果有多个）。
"""
...
import torch
import numpy as np
from scipy.signal import savgol_filter          # conda install -c conda-forge scipy
from segment_anything import sam_model_registry, SamPredictor

# ======================== 配置区 ========================
IMAGE_PATH      = r"000350.jpg"                   # 留空时弹窗选图
# CHECKPOINT_PATH = r"sam_vit_b_01ec64.pth"
CHECKPOINT_PATH = r"sam_finetuned.pth"
MODEL_TYPE      = "vit_b"
DEVICE          = "cuda" if torch.cuda.is_available() else "cpu"

OUTDIR          = "./sam_out"
VIEW_MAX_W, VIEW_MAX_H = 1600, 900
...
def smooth_poly(poly, win=15, order=3):
    if len(poly)<5: return poly.astype(np.float32)
    k = min(win|1, len(poly)//2*2+1)
...
def set_params_for_mode(m):
    ...
    if m: SMOOTH_PENALTY,MAX_STEP,PRIOR_WEIGHT,EDGE_GRAD_MIN=1.6,2,0.04,8
    else: SMOOTH_PENALTY,MAX_STEP,PRIOR_WEIGHT,EDGE_GRAD_MIN=0.6,6,0.05,10
set_params_for_mode(ROUND_MODE)

def main():
    # —— 加载图像 & 预览 ——
    img_path = IMAGE_PATH.strip() or pick_file("选择输入图像", [("Images","*.jpg;*.png")])
    if not img_path or not os.path.isfile(img_path): raise FileNotFoundError("图像文件无效")
    ...
    img_view = cv2.resize(img_bgr,(vw,vh),interpolation=cv2.INTER_AREA) if scale<1 else img_bgr.copy()
    print(f"[信息] 原图 {W}×{H}，预览 {vw}×{vh}，scale={scale:.3f}")

    # —— 初始化 SAM ——
    ckpt = CHECKPOINT_PATH.strip() or pick_file("选择 SAM 权重(.pth)", [("*.pth","*.pth")])
    sam = sam_model_registry[MODEL_TYPE](checkpoint=ckpt).to(DEVICE)
    predictor = SamPredictor(sam); predictor.set_image(img_rgb)
    ...
    while True:
        ...
        if k in (27,ord('q'),ord('Q')):
            ...
            global ROUND_MODE
            ROUND_MODE = not ROUND_MODE; set_params_for_mode(ROUND_MODE)
        ...
        # S：SAM 多掩码分割
            ...
            pc = np.array(points,np.float32) if points else None
            pl = np.array(labels,np.int32) if labels else None
            bx = np.array(box,np.float32)[None,:] if box is not None else None
            masks, scores, _ = predictor.predict(
                point_coords=pc, point_labels=pl,
                box=bx, multimask_output=True
            )
            idx = int(scores.argmax())
            last_mask = (masks[idx].astype(np.uint8) * 255)
            cv2.imshow("mask",
                cv2.resize(last_mask,(vw,vh),interpolation=cv2.INTER_NEAREST) if scale<1 else last_mask)
        ...
        # K：最平滑曲线
            ...
            cv2.imshow("edge_curve",
                cv2.resize(vis2,(vw,vh),interpolation=cv2.INTER_AREA) if scale<1 else vis2)
        ...
        # E：原 DP 折线
            ...
            cv2.polylines(msk,[np.array(poly,np.int32)],False,255,DRAW_THICKNESS)
            vis_l = img_bgr.copy()
            cv2.polylines(vis_l,[np.array(poly,np.int32)],False,(0,255,255),DRAW_THICKNESS)
            ensure_dir(OUTDIR)
            np.savetxt(os.path.join(OUTDIR,"edge_polyline.csv"),
                       np.array(poly),fmt="%d",delimiter=",",header="x,y",comments="")
            ...
            cv2.imshow("edge_only",
                cv2.resize(vis_l,(vw,vh),interpolation=cv2.INTER_AREA) if scale<1 else vis_l)
